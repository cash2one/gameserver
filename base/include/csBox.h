...
struct stGetCowBoxInitCmd : public stCowBoxUserCmd
{
	...
	//uint32_t itemID[17];  //暂时用ID来获取ObjectBase_t数据，后期根据服务端发送的物品数据类型来跟换
	t_Object objects[17];
...
struct stNewMakeObjectReturnUserCmd : public stReMakUserCmd
{
	...
	uint16_t returnNum; //sky 返回错误码(0:成功 1:钱不够 2:材料不够 3:包袱空间不足 4:未知错误)
};
/************************************************************************/
/*   sky 打造消息定义 end                                               */
/************************************************************************/
...
struct stNpcGiveItemUserCmd : public stMakeObjectUserCmd
{
	...
	uint32_t	UserTemID;	//被分配人的唯一ID
	uint32_t	qwThisID;	//被分配物品的唯一ID
};
...
struct stTeamRollItemStartUserCmd : public stMakeObjectUserCmd
{
	...
	uint32_t itemX;	//要被ROLL的物品的坐标点X
	uint32_t itemY;	//要被ROLL的物品的坐标点Y
};
...
{
	stItemCoolTimesUserCmd(uint16_t Count = 0)
	{
		byParam = ITEM_COOL_TIMES_USERCMD;
		DataCount = Count;
	}
	uint16_t DataCount;  //数据个数
};
...
struct stTeamAllNPCFlagUserCmd: public stChatUserCmd
{
	stTeamAllNPCFlagUserCmd()
	{
		...
		memset( dwFlag, 0xff, sizeof( uint32_t ) * 10 );
	}
	uint32_t dwFlag[10];  //sky 线性标志存放NPC的唯一ID
};
...
struct stTeamSetNPCFlagUserCmd: public stChatUserCmd
{
	...
	uint32_t dwNPCID; //sky 被设置NPC的唯一ID
};
//lxb end
...
{
	uint32_t UserID;			//sky 用户ID(type为队伍时候该ID为队伍唯一ID)
	BYTE  Type;				//sky 排队类型(0:个人 1:队伍)
	uint16_t  AddMeType;		//sky 排队的战场类型
